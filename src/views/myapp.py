...
class MyApp(customtkinter.CTk):
    def __init__(self, controller):
        ...
        self.topbar_frame = customtkinter.CTkFrame(self, height=60, corner_radius=0)
        ...
        self.window_frame = customtkinter.CTkScrollableFrame(self, fg_color="transparent")
        self.window_frame.grid(row=1, column=0, sticky="nsew")  # Sticky to fill the whole window

        # Ensure the grid expands in both row and column
        self.grid_rowconfigure(1, weight=15, uniform="equal")  # Row 0 will take all vertical space
        self.grid_columnconfigure(1, weight=0, uniform="equal")  # Column 0 will take all horizontal space
        ...
        self.create_area_2(self.controller,hazard_warnings,precaution_warnings)
    ...
    def create_area_1(self,table_name):
        ...
        self.area_1_header = customtkinter.CTkLabel(self.area_1, text="Chemical Inventory", font=customtkinter.CTkFont(size=18, weight="bold"))
        ...
        for i, (key, column_list) in enumerate(table_columns_dict.items()):
            ...
            for j, column in enumerate(column_list):
                ...
                label = customtkinter.CTkLabel(self.area_1, text=formatted_table_columns[j])
    ...
    def create_area_2(self, controller,hazards,precautions):
        ...
        #Create hazard frame
        self.area_2_header = customtkinter.CTkLabel(self.area_2, text="Hazard Details", font=customtkinter.CTkFont(size=18, weight="bold"))
        self.area_2_header.grid(row=0, column=0, padx=10, pady=10, sticky="w")
        self.hazard_frame = HazardPrecautionFrame(
            self.area_2, controller, warning_dict=hazards, root=self, images=False
        )
        self.hazard_frame.grid(row=1, column=0, columnspan=4,padx=10, pady=10, sticky="ew")
        self.hazard_frame.grid_columnconfigure(4,weight=1)

        #Create precautions frame        
        self.area_2_header2 = customtkinter.CTkLabel(self.area_2, text="Precautionary Details", font=customtkinter.CTkFont(size=18, weight="bold"))
        self.area_2_header2.grid(row=2, column=0, padx=10, pady=10, sticky="w")
        self.precautions_frame = HazardPrecautionFrame(
            self.area_2, controller, warning_dict=precautions, root=self, images=False
        )
        self.precautions_frame.grid(row=3, column=0, columnspan=4,padx=10, pady=10, sticky="ew")
    ...
    def create_area_3(self, controller, diamonds):
        self.area_3 = customtkinter.CTkFrame(self.window_frame, corner_radius=10)
        ...
        self.area_3_header = customtkinter.CTkLabel(self.area_3, text="Hazard Symbols", font=customtkinter.CTkFont(size=18, weight="bold"))
        self.area_3_header.grid(row=0, column=0, padx=10, pady=10, sticky="w")
        self.hazard_diamonds_frame = HazardPrecautionFrame(
            self.area_3, controller, warning_dict=diamonds, root=self, images=True
        )
        self.hazard_diamonds_frame.grid(row=1, column=0, columnspan=4,padx=10, pady=10, sticky="ew")
    ...
    def create_area_4(self):
        self.area_4 = customtkinter.CTkFrame(self.window_frame, corner_radius=10)
        ...
        self.area_4_header = customtkinter.CTkLabel(self.area_4, text="Preview", font=customtkinter.CTkFont(size=18, weight="bold"))
        self.area_4_header.grid(row=0, column=0, padx=10, pady=10, sticky="w")
        self.orientation_option_menu = customtkinter.CTkOptionMenu(self.area_4, values=["Landscape","Portrait"], command=self.update_frame)
        self.orientation_option_menu.grid(row=1, column=0, sticky="nsew", padx=10, pady=10)
    ...
    def create_area_5(self):
        self.area_5 = customtkinter.CTkFrame(self.window_frame, corner_radius=10)
        ...
        self.area_5_header = customtkinter.CTkLabel(self.area_5, text="Print", font=customtkinter.CTkFont(size=18, weight="bold"))
    ...
    def update_frame(self,selection):
        ...
        if selection == "Landscape":
            self.preview_label_frame.configure(width=400, height=300)  # Set landscape dimensions
            self.text_box.configure(width=400)
            label = customtkinter.CTkLabel(self.preview_label_frame, text="Landscape Mode", font=("Arial", 16))
            self.preview_label_frame.grid(row=2, column=0, sticky="nsew", padx=10, pady=10)
...
class HazardPrecautionFrame(customtkinter.CTkFrame):
    def __init__(self, parent, controller, warning_dict, root, images=False):
        ...
        self.warning_type_var = customtkinter.StringVar(value=first_key)  # Default value

        #Hazard dropdown menu
        self.warning_type_menu = customtkinter.CTkComboBox(
            self, variable=self.warning_type_var, values=list(warning_dict.keys()), command=self.on_dropdown_change
        )
        self.warning_type_menu.grid(row=0, column=1, padx=10, pady=5, sticky="ew")
        self.grid_columnconfigure(1, weight=3)
        
        #Exit dropdowns
        self.close_button = customtkinter.CTkButton(self, text="Close", command=self.close_checkboxes)
...
class WarningClassCheckboxes(customtkinter.CTkScrollableFrame):
    def __init__(
        ...
        if default_class:
            self.grid(row=1, column=0, columnspan=5, padx=10, pady=5, sticky="ew")
    ...
    def generate_diamonds(self, images=False):
        for item in self.warning_items:
            if images:
                ...
                image = ImageTk.PhotoImage(resized_image)  # Convert to a PhotoImage object
                item_text = item[0]
            else:
                image = None
                item_text = item[0]
            ...
            image_label.pack(side="left", padx=5)  # Pack the image label next to the checkbox
