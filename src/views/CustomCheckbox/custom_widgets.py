...
class IconCheckBox(CTkBaseClass):
    """
    ...
    def __init__(self,
                 master: any,
                 checked_icon: Union[str, CTkImage],
                 unchecked_icon: Union[str, CTkImage],
                 dark_checked_icon: Optional[CTkImage] = None,  # Add icons for dark theme
                 dark_unchecked_icon: Optional[CTkImage] = None,
        ...
        # color
        self._fg_color = ThemeManager.theme["CTkLabel"]["fg_color"] if fg_color is None else self._check_color_type(
        ...
        self._hover_color: Union[str, Tuple[str, str]] = ThemeManager.theme["CTkButton"][
            "hover_color"] if hover_color is None else self._check_color_type(hover_color)
        self._border_color: Union[str, Tuple[str, str]] = ThemeManager.theme["CTkButton"][
        ...
        self._original_color: Union[str, Tuple[str, str]] = ThemeManager.theme["CTkFrame"]["fg_color"]
        ...
        self._corner_radius: int = ThemeManager.theme["CTkButton"]["corner_radius"] if corner_radius is None \
            else corner_radius
        self._corner_radius = min(self._corner_radius, round(self._current_height / 2))
        self._border_width: int = ThemeManager.theme["CTkButton"][
        ...
        # image
        self.dark_checked_icon = self._check_image_type(dark_checked_icon, make_compatible=not auto_dark_icon)
        self.dark_unchecked_icon = self._check_image_type(dark_unchecked_icon, make_compatible=not auto_dark_icon)
        self.checked_icon = self._check_image_type(checked_icon, make_compatible=not auto_dark_icon)
        self.unchecked_icon = self._check_image_type(unchecked_icon, make_compatible=not auto_dark_icon)
        ...
        try:
            self.checked_icon = CTkImage(self.checked_icon)
            self.unchecked_icon = CTkImage(self.unchecked_icon)
        except:
            warnings.warn("Could not convert images to type CTkImage. Try using a directory instead.")

        if isinstance(self.checked_icon, CTkImage):
            self.checked_icon._dark_image = self.dark_checked_icon
            self.checked_icon.add_configure_callback(self._update_image)
        if isinstance(self.unchecked_icon, CTkImage):
    ...
    def _check_image_type(self, image: any, make_compatible=True):
        ...
        if image is None:
            ...
            if make_compatible:
                return CTkImage(Image.open(image), size=self._size)
            else:
                return Image.open(image)
        elif type(image) == Image:
            if make_compatible:
                return CTkImage(image, size=self._size)
            else:
                return image
            ...
            warnings.warn(
                ...
                f"scaled on HighDPI displays, use CTkImage or directory instead.\n")
            return image
    ...
    def _on_leave(self, event=0):
        ...
        self._label.config(cursor="", bg=self._apply_appearance_mode(fg_color))
        if self._check_state is True:
            if self._fg_color == "transparent":
    ...
    def toggle(self, event=0):
        if self._state == tkinter.NORMAL:
            ...
            if self._variable is not None:
                ...
                self._variable.set(self._on_value if self._check_state is True else self._off_value)
    ...
    def _update_image(self):
        ...
        if isinstance(new_image, CTkImage):
            self._label.configure(image=new_image.create_scaled_photo_image(self._get_widget_scaling(),
